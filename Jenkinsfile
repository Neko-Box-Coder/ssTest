...
def REPO_NAME = "ssTest"
def TARGET_URL = 'https://github.com/Neko-Box-Coder/ssTest.git'
def STATUS_CONTEXT_URL = "https://ci.nekoboxcoder.dev/job/ssTest/${BUILD_NUMBER}/pipeline-graph/"
            ...
            parallel 
            {
                stage('Linux Build') 
                {
                    ...
                    steps 
                    {
                        ...
                        bash "ls -lah"
                        bash "mkdir Build"
                        bash 'cd ./Build && cmake .. -DssTEST_BUILD_CHECK=ON && cmake --build . -j 16'
                        stash 'linux_build'
                    }
                    post { failure { script { FAILED_STAGE = env.STAGE_NAME } } }
                }
                stage('Windows Build') 
                {
                    ...
                    steps 
                    {
                        ...
                        bat 'dir'
                        bat "mkdir Build"
                        bat 'cd .\\Build && cmake .. -DssTEST_BUILD_CHECK=ON && cmake --build . -j 16'
            ...
            parallel 
            {
                stage('Linux Test') 
                {
                    ...
                    steps 
                    {
                        ...
                        bash "chmod +x ./Build/ssTestCodeFormatTest"
                        bash "cd ./Build && ./ssTestCodeFormatTest"
                    }
                    post { failure { script { FAILED_STAGE = env.STAGE_NAME } } }
                }
                ...
                stage('Windows Test') 
                {
                    ...
                    steps 
                    {
                        ...
                        bat 'cd .\\Build\\Debug && .\\ssTestCodeFormatTest.exe'
                    }
                    post { failure { script { FAILED_STAGE = env.STAGE_NAME } } }
                }
            }
